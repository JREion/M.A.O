...
# trainer name: MAO_PromptSRC_New
# NOTE: NEED TO RUN FINE-TUNING ON BASE FIRST (MAO_PromptSRC_Base)
...
import sys
from trainers import promptsrc_stats
...
# [MAO_New] Load pre-tuned weights from BASE-class fine-tuning of MAO
def load_pretrained_weights_for_mao(model, weight_path, mao=None):
    ...
    for k, v in state_dict.items():
        ...
        if k in model_dict and model_dict[k].size() == v.size():
            if mao == "mao" and k != "prompt_learner.token_prefix" and k != "prompt_learner.token_suffix":
                new_state_dict[k] = v
    ...
    if mao == "mao":
        ...
        if "prompt_learner.token_prefix" in new_state_dict:
            print("[MAO_New] Deleting prompt_learner.token_prefix in state_dict")
            del new_state_dict["prompt_learner.token_prefix"]

        if "prompt_learner.token_suffix" in new_state_dict:
            print("[MAO_New] Deleting prompt_learner.token_suffix in state_dict")
            del new_state_dict["prompt_learner.token_suffix"]
...
# [MAO_New] Call the original CLIP to get similarity scores on the NEW classes
class InferCLIP(nn.Module):
    ...
    def forward(self, image):
        with torch.no_grad():
            ...
            text_features = self.clip_model.encode_text(self.tokenized_prompts.to(image_features.device))
...
class MAO_PromptSRC_New(TrainerX):
    ...
    def build_model(self):
        ...
        # [MAO_New] Load a raw CLIP model
        infer_clip_model = load_mao_clip_to_cpu(cfg)
        ...
        if self.cfg.DATASET.SUBSAMPLE_CLASSES == "new":
            # Alterable Regularization: Use new classes as candidates
            print("[MAO_New] Tuning on [NEW], using unlabeled [NEW] images in CustomCLIP")
            self.model = CustomCLIP(cfg, classnames, clip_model)
            if self.model.training is True:
                self.infer_model = InferCLIP(cfg, classnames, infer_clip_model)
                self.infer_model.to(self.device)
                self.infer_model.eval()
                # [MAO_New] Init inference backbone of PromptSRC
                self.inference_trainer = self.build_inference_backbone(cfg)
            else:
                print("[MAO] Inference")
        else:
            print("[MAO_New] WARNING: THIS TRAINER IS [NOT] USED FOR [BASE] TASK TUNING!")
            self.model = CustomCLIP(cfg, classnames, clip_model)
        ...
        if cfg.MODEL.INIT_WEIGHTS:
            load_pretrained_weights_for_mao(self.model, cfg.MODEL.INIT_WEIGHTS, mao="mao")
    ...
    def forward_backward(self, batch):
        if self.model.training:
            image, label = self.parse_mao_batch_train(batch)  # [MAO_New] Return image + pseudo-label ids
        else:
    ...
    def build_inference_backbone(self, cfg):
        ...
        print("==== [MAO_PromptSRC] build a seprate PromptSRC backbone model for inference ====")
        sys.stdout = stop_console()
        trainer = build_trainer(cfg, name="PromptSRCStats")
        model_dir = cfg.MAO.BASE_CKPT
        trainer.load_model(model_dir, epoch=cfg.MAO.BACK_CKPT_EPOCH)
        trainer.set_model_mode("eval")
        sys.stdout = original_stdout

        return trainer

    def parse_mao_batch_train(self, batch):
        """
        [MAO_New] Data-Driven Enhancement:
        For the logits obtained by Alterable Regularization on the new classes, take the maximum value (the one
        with the highest image-text similarity) as the pesudo label, and assign the id based on this
        """
        # NOTE: Load UNLABELED IMAGE ONLY to avoid data leakage
        input_ = batch["img"]
        input_ = input_.to(self.device)

        # For the entire mini-batch of images, use the PromptSRC backbone for inference
        # text_feats: [n_cls, 512]; img_feats: [bs, 512]; batch_similarity: [bs, n_cls]
        text_feats_, img_feats_ = self.inference_trainer.model_inference(input_.type(self.model.dtype).to(self.device))
        batch_similarity = 100.0 * img_feats_ @ text_feats_.t()
        _, max_id = torch.max(batch_similarity, dim=1)
        label = max_id.to(self.device)
